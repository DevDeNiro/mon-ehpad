...
final readonly class Handler implements CoreHandler
{
    ...
    {
        $pendingOneTimePassword = $this->pendingOneTimePasswordRepository->findByOneTimePassword(OneTimePassword::create($input->oneTimePassword));

        if ($pendingOneTimePassword->isExpired()) {
            throw OneTimePasswordException::pendingOneTimePasswordExpires($pendingOneTimePassword);
        }

        $user = $this->userRepository->findByEmail(Email::create($input->email));
