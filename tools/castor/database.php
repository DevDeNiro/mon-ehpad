...
function dbSchema(#[AsOption] ?string $env = 'dev'): void
{
    ...
    run('symfony php bin/console doctrine:database:drop --if-exists -f', [
        'XDEBUG_MODE' => 'off',
        'APP_ENV' => $env,
    ]);
    run('symfony php bin/console doctrine:database:create', [
        'XDEBUG_MODE' => 'off',
        'APP_ENV' => $env,
    ]);
    run('symfony php bin/console doctrine:migration:migrate --no-interaction --allow-no-migration', [
        'XDEBUG_MODE' => 'off',
        'APP_ENV' => $env,
    ]);
}
...
function dbMigration(): void
{
    ...
    run('symfony php bin/console make:migration', [
        'XDEBUG_MODE' => 'off',
    ]);
}
